...
for page_number in range(1, PAGES_COUNT + 1):
    ...
    for game_url in games_urls:
        ...
        game_reviews_count = get_reviews_count(game_html)
        name = get_game_name(game_html)

        print("creating tags list...")
        tags_list = TagsList().from_names_list(tags)
        print("creating game...")
        game = Game(name=name, tags=tags_list, reviews_count=game_reviews_count, steam_url=game_url)
        print("adding to db...")
        db.add_game(game)
